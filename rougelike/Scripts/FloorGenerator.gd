...
func place_special_rooms():
	...
	elif normal_rooms.size() > 0:
		var boss_pos = normal_rooms[randi() % normal_rooms.size()]
		LevelManager.rooms[boss_pos].type = "boss"
		normal_rooms.erase(boss_pos)
		print("Boss room at ", boss_pos, " (non-dead-end)")
	else:
		print("ERROR: No rooms for boss")
	...
	if LevelManager.dead_ends.size() > 0:
		...
		var treasure_pos = normal_rooms[randi() % normal_rooms.size()]
		LevelManager.rooms[treasure_pos].type = "treasure"
		normal_rooms.erase(treasure_pos)
		print("Treasure room at ", treasure_pos, " (non-dead-end)")
	else:
		print("ERROR: No rooms for treasure")
	...
	if LevelManager.dead_ends.size() > 0:
		...
		var shop_pos = normal_rooms[randi() % normal_rooms.size()]
		LevelManager.rooms[shop_pos].type = "shop"
		print("Shop room at ", shop_pos, " (non-dead-end)")
	else:
		print("ERROR: No rooms for shop")
